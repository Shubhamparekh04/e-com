...
let cardOuter = document.querySelector('.card-outer');
let btnSm = document.querySelector('.btn-sm');
...
<div class="col">
    <div class="card shadow-sm">
        <img src="${ele.thumbnail}" class="card-img-top" alt="Thumbnail">
        <div class="card-body">
            <a href="#" class="h5 text-decoration-none">${ele.title}</a>
            <p class="card-text text-muted mt-2">${ele.brand?ele.brand:""}</p>
            <p class="card-text text-muted mt-2">${ele.description}</p>
            <div class="d-flex align-items-center mt-3">
                ${(() => {
                let stars = '';
                let l = 5 - Math.floor(ele.rating);
                for (let i = 0; i < Math.floor(ele.rating); i++) {
                    stars += `<div class="text-warning"><i class="bi bi-star-fill"></i></div>`;
                }
                for (let i = 1; i <= l; i++) {
                    stars += `<div class="text-warning"><i class="bi bi-star"></i></div>`;
                }
                return stars;
            })()}
                <span class="badge bg-secondary ms-2">${ele.rating}</span>
            </div>
        </div>
        <div class="card-footer d-flex justify-content-between align-items-center">
            <div>
                <del class="text-muted">$${ele.price}</del>
                <span class="text-danger fw-bold ms-2">$${(ele.price - Math.round((ele.price * ele.discountPercentage) / 100)).toFixed(2)}</span>
            </div>
            <button class="btn btn-primary btn-sm" onclick="addCart(${ele.id})">
...
//Add Cart button
let addCart = (id) => {
    let singleItem = products.find(ele => ele.id == id);
    let cartData = getMyCartData();

    let isExist = cartData.find(ele => ele.id == singleItem.id);

    if (isExist) {
        isExist.qty = isExist.qty + 1;
    } else {
        singleItem.qty = 1;
        cartData.push(singleItem);
    }
    localStorage.setItem('myCart', JSON.stringify(cartData));
    myCart = getMyCartData();
    badge.textContent = myCart.length;
}
